...
alter table `package_detail_reviews`
    add constraint `FK_package_detail_review_package` foreign key (package_detail_id) references package_details (`id`);
alter table `package_detail_reviews`
    add constraint `FK_package_detail_review_user` foreign key (user_id) references user (`id`);
...
VALUES ('Pet Care Center', '1234567890', 'petcarecente@gmail.com.com', '123 Main St', 1),
       ('Animal Hospital', '9876543210', 'animalhospital@gmail.com', '456 Elm St', 10),
...
alter table `favorite_products`
    add constraint `FK_favorite_products_products` foreign key (product_id) references product (`id`);

alter table `favorite_products`
    add constraint `FK_favorite_products_favorites` foreign key (favorite_id) references favorites (`id`);
